...
| `deliveryMode` | `'queue'` \| `'steer'` | Optional, default `queue`. Unscheduled **Codex** and **Pi** sends preserve explicit `steer`; other flavors normalize to `queue`. Codex `queue` input waits for the active turn to finish. Explicit Codex `steer` with a `localId` wakes the runner and steers compatible input into the active turn, acknowledging only positive acceptance; mode-mismatched or isolated input waits. Without an active turn it starts normally. Scheduled sends and deferred/replayed delivery (reconnect backfill, retries, scheduled release) remain `queue` and do not automatically steer a later turn. This is a delivery-intent contract, not a claim of Codex Desktop UI parity. |
